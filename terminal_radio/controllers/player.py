...
class AudioStreamer:
    ...
    def __init__(self):
        ...
        self._max_latency_samples = 10  # Keep last 10 samples for averaging
        ...
        try:
            with sd.OutputStream(
                ...
                while self._is_playing and self._process:
                    ...
                    # Calculate and update rolling average latency
                    current_latency = (time.time() - chunk_start_time) * 1000
                    self._latency_samples.append(current_latency)
                    if len(self._latency_samples) > self._max_latency_samples:
                        self._latency_samples.pop(0)
                    self._latency = sum(self._latency_samples) / len(
                        self._latency_samples
                    )
